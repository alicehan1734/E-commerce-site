...
if(isset($_GET["caselength"])){ // 페이지 마다 보여주는 창 
    ...
    $record_per_page = 5; // 15장씩 보여줄 예정 

    $start_from = ($page-1)*$record_per_page; // 첫 시작 페이지 시점 
    $sql = "SELECT * FROM address WHERE honor = '$id' ORDER BY number DESC LIMIT $start_from, $record_per_page"; // 숫자가 큰거에서 10개 만 추리기 
    ...
    //    print_r($image_datas); // 이미지의 데이터 배열 
       ...
       echo json_encode($datas);
        ...
        $sql = "SELECT * FROM address WHERE honor = '$id' ORDER BY number DESC"; // 숫자가 큰거에서 10개 만 추리기 
