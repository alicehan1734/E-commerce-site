...
$sql = "UPDATE Main SET type = '$type', price ='$price', saleprice='$saleprice', salept='$salespercentage', date='$time', info='$editor', name='$casename' WHERE id = '$getid'";
    ...
    if($conn->query($sql)){
        ...
        $result = $conn->query($sqli);
            ...
            if(!empty($fileNames)){ 
                ...
                if(!empty($insertValuesSQL)){ 
                    ...
                    $sql = "DELETE FROM listimage WHERE number = '$getid'"; // 이미지 삭제하기 
                    $conn->query($sql);
            ...
            if(is_array($typelist)){ // 리스트 저장하는 구간 
                ...
                $sql = "DELETE FROM listtype WHERE number = '$getid'"; // 리스트타입 삭제하기
                $conn->query($sql);
        ...
        $sqli = "SELECT * FROM Main WHERE date = '$time' "; // 해당 테이블의 아이디 값을 찾기위해서 
        $result = $conn->query($sqli);
         print "<script language=javascript> alert('수정 완료 되었습니다.');  location.replace('/7MakeItem/ManageList.html'); </script>";
    }else{
